...
namespace TDJS_Vision.Node._7_ResultProcessing.ResultSummarize
{
    public partial class ParamFormSummarize : FormBase, INodeParamForm
    {
        ...
        /// </summary>
        /// <returns></returns>
        public AlgorithmResult GetResult(int num)
        {
            try
            {
                NodeSubscription nodeSub = null;
                switch (num)
                {
                    case 1:
                        nodeSub = nodeSubscription1;
                        break;
                    case 2:
                        nodeSub = nodeSubscription2;
                        break;
                    case 3:
                        nodeSub = nodeSubscription3;
                        break;
                    case 4:
                        nodeSub = nodeSubscription4;
                        break;
                    default:
                        break;
                }
                switch (nodeSub.GetNodeType())
                {
                    case NodeType.AITD:
                    case NodeType.UnsupervisedDetection:
                    case NodeType.LargeModelDetection:
                        return nodeSub.GetValue<AlgorithmResult>();
                    case NodeType.SharedVariable:
                        var res = nodeSub.GetValue<SharedVarValue>();
                        return res?.Data as AlgorithmResult;
                    default:
                        throw new InvalidOperationException("不支持的节点类型");
                }
            }
            catch (Exception)
            {
