...
namespace TDJS_Vision.Node._3_Detection.TDAI.Yolo8
{
    ...
    public struct Box
    {
        ...
        public Box(PointF topLeft, PointF topRight, PointF bottomLeft, PointF bottomRight)
        {
            TopLeft = topLeft;
            TopRight = topRight;
    ...
    public struct RotatedRect
    {
        ...
        public RotatedRect(ObbResult obbResult)
        {
            // 初始化中心点为Rect的中心
            this.center = new Point2f(obbResult.center_x, obbResult.center_y);

            // 初始化尺寸为Rect的宽度和高度
            this.size = new Size2f(obbResult.width, obbResult.height);

            // 使用传入的角度，或者默认为0度
            this.angle = obbResult.angle;
        }
    }
