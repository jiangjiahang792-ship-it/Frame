...
namespace TDJS_Vision.Node._3_Detection.TDAI.Parse
{
    ...
    /// 10类多端子模型
    /// </summary>
    public class MultiTerminalParse
    {
        /// <summary>
        /// 多端子检测项中文名到内部键的映射；代码判断使用中文，运行存储仍使用内部键。
        /// </summary>
        private static readonly Dictionary<string, string> 检测项中文名到内部键 = new Dictionary<string, string>
        {
        ...
        public static void Parse(List<DetResult> results, int resultCount, NodeParamTDAI param, Process process, ref NodeResultTDAI nodeResult)
        {
            ...
            foreach (var item in deteItems)
            {
                ...
                string itemName = 获取检测项中文名(itemKey);
                ...
                bool countLimitOk = IsFlyingWireItem(itemKey)
                    ? true
                ...
                if (isCountItem)
                {
                    ...
                    {
                        if (!_autoStudyDatas.ContainsKey(itemKey))
                            _autoStudyDatas.Add(itemKey, new List<float>());
                        _autoStudyDatas[itemKey].Add(count);
                    }
                    ...
                    var isEnable = ParseCommon.GetItemEnableStatus(deteItems, itemKey);
                    ...
                    // 设置当前检测项的值到结果中
                    ParseCommon.SetDetectItemCurValue(ref param, itemKey, valueStr);
                ...
                else
                {
                    ...
                    foreach (var result in matchedResults)
                    {
                        ...
                        if (param.IsAutoStudy)
                        {
                            if (!_autoStudyDatas.ContainsKey(itemKey))
                                _autoStudyDatas.Add(itemKey, new List<float>());
                            _autoStudyDatas[itemKey].Add(value);
                        }
                    ...
                    if (matchedResults.Count>0)
                    {
                        // -------- 添加文本显示 --------0
                        texts.Add(new ColorText(
                            $"{itemName}: {(matchedResults.Count > 0 ? valueStr : "0")} " +
                            $"设定值: [{item.MinValue}, {item.MaxValue}]",
                            (param.IsAutoStudy ? true : allOk) ? Color.Green : Color.Red
                        ));

                        // 设置当前检测项的值到结果中
                        ParseCommon.SetDetectItemCurValue(ref param, itemKey, (matchedResults.Count > 0 ? valueStr.TrimEnd(',') : "0"));
                    }
                }
                ...
                if (!matchedResults.Any() && item.Enable && !isCountItem)
                {
                    ...
                    if (matchedResults.Count<=0)
                    {
                        ...
                        ParseCommon.SetDetectItemCurValue(ref param, itemKey, "0");
                        hasData = true;
                    }
                ...
                if (hasData)
                {
                    detectResults[itemKey] = singleResults;
                }
            }
        ...
        private static DetectItemInfo 获取端子数量检测项(List<DetectItemInfo> detectItemInfos)
        {
            ...
            return detectItemInfos.FirstOrDefault(item =>
                item != null &&
                item.Enable &&
        ...
        /// 判断当前检测项是否为飞丝检测项。
        /// </summary>
        /// <param name="itemKey">检测项内部键。</param>
        /// <returns>是飞丝检测项时返回 true。</returns>
        private static bool IsFlyingWireItem(string itemKey)
        {
            return string.Equals(itemKey, 飞丝内部键, StringComparison.Ordinal);
        }

        /// <summary>
        /// 根据内部检测项键获取中文名称，便于解析分支保持中文可读。
        /// </summary>
        /// <param name="itemKey">检测项内部键。</param>
        /// <returns>检测项中文名；未登记时返回原始内部键，方便定位缺失配置。</returns>
        private static string 获取检测项中文名(string itemKey)
        {
            foreach (var pair in 检测项中文名到内部键)
            {
                if (pair.Value == itemKey)
                    return pair.Key;
            }
