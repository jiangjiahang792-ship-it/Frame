    ...
    public partial class NodeParamFormIf : FormBase, INodeParamForm
    {
        ...
        public bool GetCondition()
        {
            try
            {
                ...
                {
                    case NodeType.AITD:
                        // IF节点只关心AI总结果，兼容旧方案中误保存为“输出图像”的订阅项。
                        var aiResult = nodeSubscription1.GetSelectedResult<NodeResultTDAI>().AlgorithmResult;
                        if (aiResult == null)
                            throw new Exception("AI输出结果为空，无法进行条件判断！");
                        result = aiResult.IsAllOk;
                        break;
                    ...
                    case NodeType.RGBDiscern:
                        var rgbVar = nodeSubscription1.GetValue<NodetColorResult>();
                        if (rgbVar.IsAllOk.GetType() == typeof(bool))
                            result = rgbVar.IsAllOk;
                        else
                            throw new Exception($"该共享变量类型不支持条件判断！当前类型：{rgbVar.IsAllOk.GetType()}");
                        break;
                    case NodeType.BinarizationAnalysis:
                        var binVar = nodeSubscription1.GetValue<NodeResultBinaryAnalysisResult>();
                        if (binVar.IsAllOk.GetType() == typeof(bool))
                            result = binVar.IsAllOk;
                        else
                            throw new Exception($"该共享变量类型不支持条件判断！当前类型：{binVar.IsAllOk.GetType()}");
                        break;
                    default:
                        break;
                }
