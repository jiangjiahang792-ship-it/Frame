        ...
        private async Task SendToModbus(AlgorithmResult aiResult, Dictionary<string, DetectItemAddress> mapping, double holdTime, bool isAutoReset, CancellationToken token, bool isRegister, bool showLog)
        {
            try
            {
                ...
                foreach (var group in grouped)
                {
                    ...
                    {
                        if (isRegister)
                            mod.Write(key.SignalAddress, valueToSend);
                        else
                            mod.Write(key.SignalAddress, valueToSend != 0);
                    }
                ...
                if (isAutoReset)
                {
                    ...
                    foreach (var group in grouped)
                    {
                        ...
                        {
                            if (isRegister)
                                mod1.Write(key.SignalAddress, (ushort)0);
                            else
                                mod1.Write(key.SignalAddress, false);
                        }
                        else if (device is IPlc plc)
                        {
                            var response = isRegister
