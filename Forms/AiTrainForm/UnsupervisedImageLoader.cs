...
{
    /// <summary>
    /// 无监督训练图片加载器，负责递归扫描图片、分类并生成缩略图。
    /// </summary>
    public static class UnsupervisedImageLoader
    {
        /// <summary>
        /// 支持加载的图片后缀。
        /// </summary>
        ...
        public static List<UnsupervisedImageItem> LoadImages(string folder, Size thumbnailSize, CancellationToken token)
        {
            if (string.IsNullOrWhiteSpace(folder) || !Directory.Exists(folder))
                return new List<UnsupervisedImageItem>();
            ...
            var result = new List<UnsupervisedImageItem>(files.Count);
            foreach (string file in files)
            {
                ...
                try
                {
                    ...
                    result.Add(new UnsupervisedImageItem
                    {
                        FilePath = file,
        ...
        private static UnsupervisedImageCategory DetectCategory(string file, string root)
        {
            ...
            foreach (string token in tokens)
            {
                ...
                if (value.Equals("NG", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    value.Contains("不良"))
                {
                    return UnsupervisedImageCategory.NG;
                }
            }

            return UnsupervisedImageCategory.OK;
        }
