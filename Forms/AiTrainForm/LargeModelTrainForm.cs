...
{
    /// <summary>
    /// 大模型训练窗体，负责图片检查、单 ROI 设置、demo 对齐参数配置和模板生成。
    /// </summary>
    public partial class LargeModelTrainForm : Form
    {
        /// <summary>
        /// 已加载的训练图片。
        /// </summary>
        private readonly List<LargeModelImageItem> _images = new List<LargeModelImageItem>();

        /// <summary>
        /// 当前已渲染的缩略图卡片索引，用于局部刷新选中边框。
        /// </summary>
        private readonly Dictionary<LargeModelImageItem, Control> _imageCards = new Dictionary<LargeModelImageItem, Control>();
        ...
        /// 大模型训练服务。
        /// </summary>
        private readonly LargeModelTrainingService _trainingService = new LargeModelTrainingService();

        /// <summary>
        /// 默认模型类别，当前 native demo 固定使用 DINOv2。
        /// </summary>
        private const string DefaultModelType = "DINOv2";

        /// <summary>
        /// 默认模型输入宽度，C# demo 固定使用 448x224 模型文件，不作为训练参数展示。
        /// </summary>
        private const int DefaultInputWidth = 448;

        /// <summary>
        /// 默认模型输入高度，C# demo 固定使用 448x224 模型文件，不作为训练参数展示。
        /// </summary>
        ...
        /// 当前分类筛选。
        /// </summary>
        private LargeModelImageCategory _currentFilter = LargeModelImageCategory.All;

        /// <summary>
        /// 当前选中的图片。
        /// </summary>
        private LargeModelImageItem _selectedImage;
        ...
        private void InitializeFormState()
        {
            ...
            numericUpDownInputWidth.Value = DefaultInputWidth;
            numericUpDownInputHeight.Value = DefaultInputHeight;
            buttonStopTraining.Enabled = false;
            imageROIEditControlPreview.RoiChanged += imageROIEditControlPreview_RoiChanged;
            textBoxTemplateName.Text = "大模型模板_" + DateTime.Now.ToString("yyyyMMdd_HHmmss", CultureInfo.InvariantCulture);
        ...
        /// </summary>
        private async void LargeModelTrainForm_Shown(object sender, EventArgs e)
        {
        ...
        /// 从最小化恢复到最大化后再次修正边界。
        /// </summary>
        private void LargeModelTrainForm_Resize(object sender, EventArgs e)
        {
            RefreshAdaptiveText();
        ...
        {
            SetFilter(LargeModelImageCategory.All);
        }
        ...
        {
            SetFilter(LargeModelImageCategory.OK);
        }
        ...
        {
            SetFilter(LargeModelImageCategory.NG);
        }
        ...
        {
            SetSelectedImageCategory(LargeModelImageCategory.OK);
        }
        ...
        {
            SetSelectedImageCategory(LargeModelImageCategory.NG);
        }
        ...
        private async void buttonStartTraining_Click(object sender, EventArgs e)
        {
            try
            {
                LargeModelTrainingRequest request = BuildTrainingRequest();
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.AddLog(MsgLevel.Exception, "大模型训练失败：" + ex.Message, true);
        ...
        private async Task LoadImagesAsync(string folder)
        {
            ...
            try
            {
                ...
                List<LargeModelImageItem> loaded = await Task.Run(() => LargeModelImageLoader.LoadImages(folder, thumbnailSize, token), token);
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.AddLog(MsgLevel.Exception, "加载大模型训练图像失败：" + ex.Message, true);
            }
            finally
            {
        ...
        private void SetFilter(LargeModelImageCategory category)
        {
        ...
        private Control CreateImageCard(LargeModelImageItem item)
        {
            var card = new LargeModelImageCardControl(item)
            {
                Width = 172,
                Height = 172,
        ...
        {
            if (!(sender is Control control) || !(control.Tag is LargeModelImageItem item)) return;

            Color color = item.Category == LargeModelImageCategory.NG ? Color.FromArgb(230, 40, 40) : Color.FromArgb(0, 190, 100);
        ...
        private void imageCard_Click(object sender, EventArgs e)
        {
            ...
            if (control?.Tag is LargeModelImageItem item)
            {
                SelectImage(item);
            }
        }
        ...
        /// </summary>
        /// <param name="item">图片信息。</param>
        private void SelectImage(LargeModelImageItem item)
        {
            LargeModelImageItem previous = _selectedImage;
            _selectedImage = item;
            try
            {
        ...
        /// </summary>
        /// <param name="category">目标分类。</param>
        private void SetSelectedImageCategory(LargeModelImageCategory category)
        {
            if (_selectedImage == null)
            {
        ...
        {
            LargeModelImageItem selected = _selectedImage;
            RenderImageCards();
            ...
            LargeModelImageItem fallback = GetFilteredImages().FirstOrDefault();
            if (fallback != null)
            {
        ...
        private LargeModelTrainingRequest BuildTrainingRequest()
        {
            if (_images.Count == 0)
            ...
            return new LargeModelTrainingRequest
            {
                ...
                Precision = comboBoxModelType.Text,
        ...
        {
            LargeModelRuntimeStatus status = LargeModelRuntimeBootstrapper.ValidateRuntime();
        ...
        private void UpdateGeneratedModelPath()
        {
            ...
            textBoxModelOutputPath.Text = Path.Combine(modelRoot, name + ".tdlarge");
        }
        ...
        {
            int okCount = _images.Count(item => item.Category == LargeModelImageCategory.OK);
            int ngCount = _images.Count(item => item.Category == LargeModelImageCategory.NG);
        ...
        private IEnumerable<LargeModelImageItem> GetFilteredImages()
        {
            return _currentFilter == LargeModelImageCategory.All
                ? _images
        ...
        private bool IsImageVisibleByCurrentFilter(LargeModelImageItem item)
        {
            return item != null && (_currentFilter == LargeModelImageCategory.All || item.Category == _currentFilter);
        }
        ...
        {
            ApplyFilterButtonStyle(buttonFilterAll, _currentFilter == LargeModelImageCategory.All);
            ApplyFilterButtonStyle(buttonFilterOk, _currentFilter == LargeModelImageCategory.OK);
            ApplyFilterButtonStyle(buttonFilterNg, _currentFilter == LargeModelImageCategory.NG);
        }
        ...
        /// </summary>
        /// <param name="item">图片信息。</param>
        private void InvalidateImageCard(LargeModelImageItem item)
        {
            if (item == null) return;
            Control card;
            if (_imageCards.TryGetValue(item, out card) && card != null && !card.IsDisposed)
            {
                if (card is LargeModelImageCardControl imageCard)
        ...
        {
            foreach (LargeModelImageItem item in _images)
            {
                item.Dispose();
            }
            _images.Clear();
        ...
        private void AppendTrainingLog(string message)
        {
            ...
            LogHelper.AddLog(MsgLevel.Info, "【大模型训练】" + message, true);
        }
        ...
        {
            string text = string.IsNullOrWhiteSpace(value) ? "大模型模板" : value.Trim();
        ...
        /// <summary>
        /// 大模型图片缩略图自绘卡片，替代 PictureBox+Label 组合以降低滚动时控件数量。
        /// </summary>
        private sealed class LargeModelImageCardControl : Control
        {
            /// <summary>
            /// 卡片对应的图片信息。
            /// </summary>
            private readonly LargeModelImageItem _item;

            /// <summary>
            /// 初始化缩略图卡片。
            /// </summary>
            /// <param name="item">图片信息。</param>
            public LargeModelImageCardControl(LargeModelImageItem item)
            {
                _item = item;
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                Color borderColor = Selected
                    ...
                    : (_item != null && _item.Category == LargeModelImageCategory.NG ? Color.FromArgb(230, 40, 40) : Color.FromArgb(0, 190, 100));
