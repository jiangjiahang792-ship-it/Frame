...
namespace TDJS_Vision.Forms.ShapeDraw
{
    public partial class ImageROIEditControl : UserControl
    {
        ...
        /// 获取当前 ROI 数量。
        /// </summary>
        /// <returns>ROI 数量。</returns>
        public int GetRoiCount()
        {
            return roiManager.ROIs.Count;
        }

        /// <summary>
        /// 进入矩形 ROI 绘制状态。
        /// </summary>
        public void BeginDrawRectangleRoi()
        {
            selectedROIType = ROIType.Rectangle;
            矩形ToolStripMenuItem1.Checked = true;
            圆形ToolStripMenuItem1.Checked = false;
            绘制状态ToolStripMenuItem.Checked = true;
        ...
        /// <summary>
        /// 按指定图像尺寸计算所有 ROI 对应的图像坐标矩形。
        /// </summary>
        /// <param name="imageWidth">图像宽度。</param>
        /// <param name="imageHeight">图像高度。</param>
        /// <returns>图像坐标系下的 ROI 矩形列表。</returns>
        public List<Rect> GetImageROIRects(int imageWidth, int imageHeight)
        {
            return roiManager.GetImageROIRects(imageWidth, imageHeight);
        }
