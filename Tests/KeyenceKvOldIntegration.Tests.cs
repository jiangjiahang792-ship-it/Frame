...
internal static class KeyenceKvOldIntegrationTests
{
    ...
    private static PLCParms Parameters(int port)
    {
        return new PLCParms
        {
            ...
            KeyenceProtocol = KeyenceProtocol.Kv300Older,
    ...
    private static void VerifyParameters()
    {
        ...
        ExpectInvalid(invalid);
        var plc = new PlcKeyenceKvOld(Parameters(8501));
    ...
    private static void ExpectInvalid(PLCParms parms)
    {
        try
        {
            new PlcKeyenceKvOld(parms).Release();
        }
    ...
    private static void VerifyAddAndPersistence(string imagePath)
    {
        ...
        try
        {
            using (var form = new FrmPLCNew())
            {
                ...
                var tab = Control<TabPage>(form, "tabPageKeyenceOld");
                Check(tab.Text == "基恩士 KV300/Older", "基恩士页标题错误。");
                ...
                Control<TextBox>(form, "textBoxNameKeyenceOld").Text = "  基恩士添加验证  ";
                Control<TextBox>(form, "textBoxPortKeyenceOld").Text = "18501";
                Check(Control<Sunny.UI.UIIPTextBox>(form, "uiipTextBoxKeyenceOld").Text == "127.0.0.1", "IP默认资源错误。");
                Control<Button>(form, "buttonConfirmKeyenceOld").PerformClick();
                Check(single != null && single.Plc is PlcKeyenceKvOld, "确认添加未创建基恩士设备。");
                Check(Solution.Instance.PlcDevices.Contains(single.Plc), "设备未进入现有PLC列表。");
                Check(single.Plc.UserDefinedName == "基恩士添加验证", "设备名称未正确保存。");
                Check(single.Plc.PLCParms.KeyenceProtocol == KeyenceProtocol.Kv300Older, "添加时协议标识错误。");
                ...
                var restored = restoredDevices[0] as PlcKeyenceKvOld;
                Check(restored != null, "保存的ClassName无法恢复基恩士类型。");
                restored.CreateDevice();
                Check(restored.PLCParms.KeyenceProtocol == KeyenceProtocol.Kv300Older, "恢复时协议标识错误。");
    ...
    private static void VerifyCommunication()
    {
        ...
        var plc = new PlcKeyenceKvOld(Parameters(port));
        ...
        try
        {
            ...
            Check(server.ReadInt32("DM100").Content == 123456, "多字写入必须完整写入高位。");
            var integerResult = plc.ReadInt32("DM100");
            Check(integerResult.IsSuccess && integerResult.Content == 123456, "读取整数错误：" + integerResult.Message + "，值=" + integerResult.Content);
            Check(plc.WriteBoolAsync("R100", true).GetAwaiter().GetResult().IsSuccess, "异步写入位失败。");
            var bits = plc.ReadBoolAsync(new[] { "R100", "R101", "R100" }).GetAwaiter().GetResult();
            Check(bits.IsSuccess && bits.Content.SequenceEqual(new[] { true, false, true }), "离散位地址顺序错误。");
            Check(plc.ReadBool("R100", 1).Content[0] &&
                plc.ReadBoolAsync("R100", 1).GetAwaiter().GetResult().Content[0], "PLC读取节点的单点数组适配失败。");
            Check(plc.WriteBytes("DM150", BitConverter.GetBytes(5000000000L)).IsSuccess &&
                plc.ReadInt64("DM150").Content == 5000000000L, "64位数据读写被截断。");
            Check(plc.WriteStringAsync("DM160", "ABCD").GetAwaiter().GetResult().IsSuccess &&
                plc.ReadStringAsync("DM160", 2).GetAwaiter().GetResult().Content == "ABCD", "字符串多字读写失败。");
            Check(!plc.WriteBytes("DM170", new byte[] { 1 }).IsSuccess, "不完整字节必须拒绝写入。");
            Check(!plc.WriteInt("TC1", 123456).IsSuccess, "非DM地址不能静默截断多字值。");
            ...
            var restored = JsonConvert.DeserializeObject<PlcKeyenceKvOld>(json);
            try
            {
                restored.CreateDevice();
                Check(restored.PLCParms.KeyenceProtocol == KeyenceProtocol.Kv300Older, "恢复时协议标识错误。");
