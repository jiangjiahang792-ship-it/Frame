...
Assert-Contains $service "okImageCount < 1" "Large-model training must allow a single OK image as the minimum training set."
Assert-Contains $service "CalibrationOkImagePath" "Large-model training must keep an OK calibration image outside the training set."
Assert-Contains $service "CalibrationNgImagePath" "Large-model training must keep an NG calibration image outside the training set."
Assert-Contains $service "CalibrateImageThreshold" "Large-model training must calibrate the recognition score after bank training."
Assert-Contains $service "HasCalibrationImages(_dataset)" "Large-model training must skip automatic score calibration when held-out samples are unavailable."
Assert-Contains $service "不影响模型生成" "Large-model score calibration failures must use the default threshold without failing training."
Assert-Contains $service "BuildManifest(_request, _dataset, modelPath, _deviceMode, _calibratedImageThreshold)" "Large-model template manifest must persist the calibrated image threshold."
