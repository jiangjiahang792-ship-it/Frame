...
$testExecutable = Join-Path $validationDirectory 'KeyenceKvOldIntegration.Tests.exe'
$references = @(
...
& $compiler /nologo /target:exe /platform:x64 /langversion:7.3 "/out:$testExecutable" @references (Join-Path $PSScriptRoot 'KeyenceKvOldIntegration.Tests.cs')
if ($LASTEXITCODE -ne 0) { throw '基恩士专项测试编译失败。' }
& $testExecutable (Join-Path $validationDirectory '基恩士旧系列添加界面.png')
if ($LASTEXITCODE -ne 0) { throw '基恩士专项测试失败。' }
