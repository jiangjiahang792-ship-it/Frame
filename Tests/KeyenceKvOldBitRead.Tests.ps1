...
param([string]$BuildDirectory = 'bin/KeyenceNanoValidation', [string]$SchemePath)
...
$testExecutable = Join-Path $validationDirectory 'KeyenceKvOldBitRead.Tests.exe'
$references = @(
...
& $compiler /nologo /target:exe /platform:x64 /langversion:7.3 "/out:$testExecutable" @references (Join-Path $PSScriptRoot 'KeyenceKvOldBitRead.Tests.cs')
if ($LASTEXITCODE -ne 0) { throw 'PLC订阅专项测试编译失败。' }
if ($SchemePath) { & $testExecutable $SchemePath } else { & $testExecutable }
