...
$testExecutable = Join-Path $validationDirectory 'KeyenceNanoIntegration.Tests.exe'
$references = @(
...
& $compiler /nologo /target:exe /platform:x64 /langversion:7.3 "/out:$testExecutable" @references (Join-Path $PSScriptRoot 'KeyenceNanoIntegration.Tests.cs')
if ($LASTEXITCODE -ne 0) { throw '基恩士专项测试编译失败。' }
& $testExecutable (Join-Path $validationDirectory '基恩士添加界面.png')
