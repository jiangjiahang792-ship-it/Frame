...
## 2026-07-20: 多目标位置修正与测量

- [x] 改造前工作区无未提交修改，已创建 Git 回退快照 `c92bd79`。
- [x] 已确认模板匹配输出全部目标位姿，当前模型的 `ScaleX`、`ScaleY` 固定为 `1.0`。
- [x] 已确认位置修正默认以第一个目标创建基准，并输出全部目标的变换集合。
- [x] 已确认测量界面只保留一个可拖动 ROI，保存或运行时自动识别 ROI 所属目标并逆变换到统一基准坐标系。
- [x] 已确认卡尺找线、卡尺找圆、卡尺找椭圆、找点、线线角度、点点距离、点线距离和点区域距离统一支持多目标运行结果。
- [x] 已确认单目标测量失败时保留同序号结果项、数值写 `0`，节点总体状态为 NG，其他目标继续测量。
- [x] 已完成设计文档：`docs/superpowers/specs/2026-07-20-multi-target-position-correction-measurement-design.md`。
- [x] 已完成详细测试驱动实施计划：`docs/superpowers/plans/2026-07-20-multi-target-position-correction-measurement.md`。
- [x] 按计划完成多目标数据模型、位置修正、八类测量工具和界面改造。
  - [x] 已新增模板目标位姿契约和可替换的多目标变换服务，覆盖尺度保护、正逆变换、旋转框命中及框外最近目标选择。
  - [x] `MultiTargetTransform.Tests.ps1` 已按测试驱动方式先失败后通过。
- [x] 完成专项测试、相关回归测试和 `Debug|x64` 编译验证。
- [x] 2026-07-20 多目标位置修正节点改造
  - [x] 增加模板位姿列表、位置修正集合及单订阅界面的失败约束测试
  - [x] 模板匹配输出全部目标位姿，缩放固定输出 1.0
  - [x] 位置修正默认第一目标为基准，并输出全部目标修正集合
  - [x] 补齐可供全部测量工具复用的公共位置修正结果契约
- [x] 2026-07-20 通用多目标测量执行器
  - [x] 增加目标失败保留、后续继续和取消透传的失败行为测试
  - [x] 实现统一目标结果接口、基类和顺序执行器
- [x] 2026-07-20 三种卡尺多目标测量
  - [x] 增加结果集合、公共执行器、ROI 归属和失败保留的失败契约测试
  - [x] 三种卡尺结果增加按目标顺序排列的强类型集合
  - [x] 卡尺找线支持唯一 ROI 归一化、尺寸缩放与多目标结果
  - [x] 卡尺找圆支持唯一 ROI 归一化、尺寸缩放与多目标结果
  - [x] 卡尺找椭圆支持唯一 ROI 归一化、各向缩放与多目标结果
  - [x] 单目标失败保留同序号全零结果，节点总体状态为 NG 且继续后续目标
- [x] 2026-07-20 找点工具多目标测量
  - [x] 增加区域逐点变换、唯一 ROI 归属和失败保留的失败契约测试
  - [x] 找点一次取图后逐目标运行并输出强类型结果集合
  - [x] 多区域按整体包围盒中心解析所属目标，全部顶点逆变换到第一目标基准
  - [x] 单目标失败保留同序号全零结果并继续后续目标
  - [x] 专项契约测试通过；禁用构建节点复用后 Debug|x64 编译通过
- [x] 2026-07-20 四种几何工具多目标测量
  - [x] 线线夹角、点点距离、点线距离和点区域距离增加按目标顺序排列的强类型结果集合
  - [x] 仅绘制模式按全部位置修正目标展开；订阅模式保持原有单结果行为
  - [x] 编辑界面只显示一套测量几何，拖到任意目标后按整体中心识别归属并逆变换到第一目标基准
  - [x] 线端点、圆心和区域顶点逐点仿射变换，圆半径与卡尺宽高按目标尺度同步缩放
  - [x] 四种工具一次读取灰度图后逐目标运行，单目标失败保留同序号全零结果并继续后续目标
  - [x] 三种卡尺补强普通算法失败分支，边缘点数和算法耗时等数值统一输出0
  - [x] `MultiTargetGeometryMeasurement.Tests.ps1` 先失败后通过；六项多目标专项测试全部通过
  - [x] `Debug|x64` 编译通过，只有项目既有警告、0个错误
- [x] 2026-07-20 多目标测量审查修正与回归收尾
  - [x] ROI归属中心统一改为整套测量几何的包围盒中心，避免顶点分布不均时误判所属目标
  - [x] 空位置修正集合直接输出空结果且总体NG，不再读取图像或保留上一帧结果
  - [x] 无效位置修正项交由多目标执行器逐项隔离，生成同序号全零失败项并继续后续目标
  - [x] 八类测量工具再次进入绘制状态前清除上一轮多目标静态叠加，只保留当前可编辑几何
  - [x] 线线夹角复用本轮共享灰度图尺寸，取消逐目标重复读取上游 `OutputImage`
  - [x] 独立代码审查复核三项问题均已解决，未发现新的严重或重要问题
  - [x] 六项多目标专项测试全部通过；`git diff --check` 通过；`Debug|x64` 完整编译通过，179个既有警告、0个错误
  - [x] 全库41项PowerShell回归测试中39项通过；`ImageSaveMultiConditionSubscription.Tests.ps1` 因既有乱码导致脚本解析失败，`UnsupervisedTrainForm.Tests.ps1` 因既有中文按钮文本断言失败，均与本次测量改动文件无交集

## 2026-07-21: 修复旧方案位置修正订阅与多目标列表不兼容

- [x] 使用用户提供的 `222.Sol` 定位根因：卡尺找圆仍保存“位置修正信息”单值订阅，新版确认 ROI 按 `List<PositionCorrectionInfo>` 读取时触发类型不匹配。
- [x] 为通用订阅控件增加期望值类型约束，只显示能够赋值给调用方目标类型的结果属性；旧单值名称不存在于筛选结果时自动回落到“位置修正信息列表”。
- [x] 卡尺找线、卡尺找圆、卡尺找椭圆、找点、线线夹角、点点距离、点线距离和点区域距离统一声明位置修正列表类型，已有方案打开后无需手工重配，保存时自动写回列表订阅名称。
- [x] 新增 `PositionCorrectionSubscriptionCompatibility.Tests.ps1`，覆盖旧单值属性排除、列表属性选择、接口类型兼容、未声明类型保持原行为，以及八类测量工具初始化顺序；测试按测试驱动方式先失败后通过。
- [x] 七项多目标专项回归测试全部通过；`git diff --check` 通过；MSBuild `Debug|x64` 完整编译通过，179 个项目既有警告、0 个错误。

## 2026-07-21: 卡尺找圆多目标共享灰度图并发

- [x] 公共多目标执行器增加目标级受限并发，单目标保持调用线程直接执行，不承担并发调度开销。
- [x] 默认最大并发度取目标数量与 CPU 逻辑核心数减一的较小值，固定结果下标保证输出数量和顺序与位置修正集合一致。
- [x] 卡尺找圆只获取一次只读灰度图，并发执行各目标的参数仿射和找圆算法，全部结束后统一汇总与绘制。
- [x] 单目标普通异常保留同位置全零 NG 项，其他目标继续执行；流程取消继续向节点运行层传播。
- [x] 节点墙钟耗时改用高精度 `Stopwatch`；汇总 `AlgorithmMs` 保持各目标累计算法工作量语义。
- [x] 公共执行器测试覆盖真实并发重叠、最大并发度、单目标同线程快路径、固定顺序、失败隔离和预取消传播；并发测试连续重复 5 次均通过。
- [x] 七项多目标专项回归测试全部通过；MSBuild `Debug|x64` 完整编译通过，179 个项目既有警告、0 个错误。
- [x] 使用 `222.Sol` 的 `Count=30`、卡尺宽 10、高 60 参数和 1280×1024 纽扣灰度图完成 40 次本地基准：5 目标串行墙钟中位数 11.837ms、P95 16.791ms，并发中位数 3.442ms、P95 4.399ms，中位数提升 3.44 倍。
- [x] 本地基准 ROI 在所选样图上未拟合出有效圆，但完整卡尺扫描路径已执行；实际成功测量耗时由用户在正式方案运行画面继续复测，不编造成功结果数据。
- [x] 公共并发执行器首次加载时读取并缓存 `Environment.ProcessorCount`；注释明确该值是 CPU 逻辑处理器数量而非物理核心数量，自动并发默认保留一个逻辑处理器且不超过目标数量。当前开发机实际读取为 24，缓存自动上限为 23。

## 2026-07-21: 卡尺快速采样与抗干扰采样双模式

- [x] 对比当前版与旧版运行日志和公共卡尺算法，确认单目标耗时回退来自宽度平均及双线性采样，不是多目标并发调度。
- [x] 确认采用每个卡尺节点独立保存采样枚举的设计，默认快速采样，抗干扰模式保留当前算法行为。
- [x] 完成设计文档：`docs/superpowers/specs/2026-07-21-caliper-sampling-modes-design.md`。
- [x] 完成测试驱动实施计划并按计划实现三种卡尺双模式；专项测试依次验证缺少公共枚举、节点参数和 Designer 控件三个失败阶段，最终转为通过。
- [x] 卡尺找线、卡尺找圆和卡尺找椭圆均增加独立保存的采样模式；快速采样使用中心线单像素最近邻读取，抗干扰采样保留宽度平均双线性读取，未知枚举值回落到快速采样。
- [x] 三种卡尺参数窗体均在对应 `.Designer.cs` 中加入“采样模式”下拉框，提供“快速采样”和“抗干扰采样”，默认选择快速采样。
- [x] 旧方案 `C:\Users\34652\Desktop\222.Sol` 不包含 `SamplingMode`；使用实际 Newtonsoft.Json 反序列化确认三种节点参数均默认得到枚举值 0，重新序列化均写出 `"SamplingMode": 0`。
- [x] 双模式及多目标测量相关八项专项测试全部通过；`Debug|x64` 完整编译通过，179 个项目既有警告、0 个错误。
- [x] 使用同一张 1280×1024 纽扣灰度图、卡尺找圆 `Count=30`、宽 10、高 60，在热身 10 次后各运行 100 次：单目标快速采样中位数 0.127ms、P95 0.146ms，抗干扰采样中位数 1.650ms、P95 1.748ms，快速模式中位数约快 13 倍，两种模式均成功得到 30 个边缘点。
- [x] 同参数五目标共享灰度图并发基准中，快速采样墙钟中位数 0.140ms、P95 0.279ms，抗干扰采样中位数 1.669ms、P95 2.356ms；两种模式每轮均得到 5 个成功结果。

## 2026-07-22: 复合测量双采样模式与多目标串行统一

- [x] 确认所有测量工具的多个模板目标统一采用串行执行，卡尺找圆也从受限并发改回串行。
- [x] 确认线线夹角、点点距离和点线距离按当前“运行ROI”分别保存两套卡尺采样模式，点区域距离保存一套点圆卡尺采样模式。
- [x] 完成设计文档：`docs/superpowers/specs/2026-07-22-composite-measurement-sampling-and-serial-design.md`。
- [x] 完成测试驱动实施计划并实现四种复合测量工具双采样模式；专项测试先因线线夹角缺少模式失败，再推进到 Designer 缺少控件，最后推进到卡尺找圆仍调用并发入口，生产代码逐层完成后全部转绿。
- [x] 线线夹角的直线1/直线2、点点距离的点1/点2、点线距离的点圆/直线分别独立保存快速或抗干扰采样；点区域距离保存点圆卡尺模式，切换“运行ROI”时同步保存和加载当前卡尺模式。
- [x] 四个复合测量窗体均在对应 `.Designer.cs` 的算法参数区域加入“采样模式”下拉框，提供“快速采样”和“抗干扰采样”。
- [x] 卡尺找圆从 `MultiTargetMeasurementRunner.RunParallel` 改回串行 `Run`；八种业务测量窗体均不再调用 `RunParallel`，公共并发执行器仅保留备用。
- [x] 使用实际程序集和 Newtonsoft.Json 反序列化空 JSON，确认七个新增采样字段均默认得到枚举值0并能序列化为0；旧 `C:\Users\34652\Desktop\222.Sol` 不包含这些字段，因此自动进入快速采样。
- [x] 复合采样、卡尺采样、位置修正和多目标测量相关九项专项测试全部通过；`Debug|x64` 非增量构建通过，179 个项目既有警告、0 个错误。
